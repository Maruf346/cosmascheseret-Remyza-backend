...
class BusinessEndUser(models.Model):
    organization = models.ForeignKey("business.Organization", on_delete=models.CASCADE, related_name="new_business_end_users")
    customer_profile = models.ForeignKey(CustomerProfile, on_delete=models.CASCADE, related_name="new_end_users", null=True, blank=True)
...
class BusinessAddress(models.Model):
    organization = models.ForeignKey("business.Organization", on_delete=models.CASCADE, related_name="new_business_addresses")
    twilio_sid = models.CharField(max_length=64, unique=True, db_index=True)
    friendly_name = models.CharField(max_length=255, blank=True, default="")
    customer_name = models.CharField(max_length=255, blank=True, default="")
    street = models.CharField(max_length=255)
    street_secondary = models.CharField(max_length=255, blank=True, default="")
    city = models.CharField(max_length=100)
    state = models.CharField(max_length=100)
    postal_code = models.CharField(max_length=30)
    country = models.CharField(max_length=10)
    last_synced_at = models.DateTimeField(null=True, blank=True)
    twilio_data = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class TrustHubEntityAssignment(models.Model):
    customer_profile = models.ForeignKey(CustomerProfile, on_delete=models.CASCADE, related_name="new_entity_assignments", null=True, blank=True)
...
class TrustHubEvaluation(models.Model):
    customer_profile = models.ForeignKey(CustomerProfile, on_delete=models.CASCADE, related_name="new_evaluations", null=True, blank=True)
    a2p_profile = models.ForeignKey(A2PProfile, on_delete=models.CASCADE, related_name="new_evaluations", null=True, blank=True)
...
class A2PBrand(models.Model):
    organization = models.ForeignKey("business.Organization", on_delete=models.CASCADE, related_name="new_a2p_brands")
    customer_profile = models.ForeignKey(CustomerProfile, on_delete=models.PROTECT, related_name="new_brands")
    a2p_profile = models.ForeignKey(A2PProfile, on_delete=models.PROTECT, related_name="new_brands")
    brand_sid = models.CharField(max_length=64, unique=True, db_index=True)
    brand_type = models.CharField(max_length=50, choices=A2PBrandType.choices, default=A2PBrandType.STANDARD)
    status = models.CharField(max_length=50, choices=A2PBrandStatus.choices, default=A2PBrandStatus.DRAFT)
    identity_status = models.CharField(max_length=100, blank=True, default="")
    failure_reason = models.TextField(blank=True, default="")
    submitted_at = models.DateTimeField(null=True, blank=True)
    approved_at = models.DateTimeField(null=True, blank=True)
    last_synced_at = models.DateTimeField(null=True, blank=True)
...
class A2PCampaign(models.Model):
    organization = models.ForeignKey("business.Organization", on_delete=models.CASCADE, related_name="new_a2p_campaigns")
    brand = models.ForeignKey(A2PBrand, on_delete=models.PROTECT, related_name="new_campaigns")
...
class MessagingService(models.Model):
    ...
    messaging_service_sid = models.CharField(max_length=64, blank=True, default="")
    last_synced_at = models.DateTimeField(null=True, blank=True)
    metadata = models.JSONField(default=dict, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class MessagingServicePhoneAssignment(models.Model):
    messaging_service = models.ForeignKey(MessagingService, on_delete=models.CASCADE, related_name="new_phone_assignments")
    phone_number = models.ForeignKey("business.PhoneNumber", on_delete=models.CASCADE, related_name="new_messaging_service_assignments")
    attachment_sid = models.CharField(max_length=64, unique=True, db_index=True)
    ...
    def __str__(self):
        return f"{self.messaging_service.service_sid} - {self.phone_number}"
...
class LocalVerification(models.Model):
    ...
    status = models.CharField(max_length=50, choices=LocalVerificationStatus.choices, default=LocalVerificationStatus.DRAFT)
